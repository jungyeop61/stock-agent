...
class OrderExecutionPersistenceTests {
	...
	@Test
	@DisplayName("수량 주문 제출 직전 안전 차단 상태를 데이터베이스에 저장한다")
	void 수량_주문_제출_직전_안전_차단_상태를_데이터베이스에_저장한다() {
		OffsetDateTime createdAt = OffsetDateTime.of(
				2026, 9, 4, 22, 30, 0, 0, ZoneOffset.ofHours(9));
		OrderPreviewResponse preview = 승인된_미리보기를_저장한다(createdAt);
		OrderExecutionResponse prepared = 실행_준비_기록을_만든다(
				preview.previewId(), createdAt.plusSeconds(10));
		executionStore.claim(prepared, "e".repeat(64));
		executionStore.markSubmitting(prepared.executionId(), createdAt.plusSeconds(11));

		boolean blocked = executionStore.markSubmissionBlocked(
				prepared.executionId(), createdAt.plusSeconds(12));
		boolean duplicate = executionStore.markSubmissionBlocked(
				prepared.executionId(), createdAt.plusSeconds(13));
		...
		assertThat(stored.status()).isEqualTo(OrderExecutionStatus.REJECTED);
		assertThat(stored.failureType()).isEqualTo(OrderExecutionFailureType.INTERNAL_STATE);
		assertThat(stored.completedAt()).isEqualTo(createdAt.plusSeconds(12));
	}
