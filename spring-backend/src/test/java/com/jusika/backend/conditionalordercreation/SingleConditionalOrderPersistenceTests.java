...
class SingleConditionalOrderPersistenceTests {
	...
	@Test
	@DisplayName("SINGLE 제출 직전 안전 차단 상태를 데이터베이스에 저장한다")
	void SINGLE_제출_직전_안전_차단_상태를_데이터베이스에_저장한다() {
		OffsetDateTime createdAt = OffsetDateTime.of(
				2026, 9, 7, 16, 0, 0, 0, ZoneOffset.ofHours(9));
		SingleConditionalOrderPreviewResponse preview = previewStore.save(
				미리보기를_만든다(createdAt));
		SingleConditionalOrderExecutionResponse prepared = 실행_준비_기록을_만든다(
		...
		SingleConditionalOrderExecutionResponse stored = executionStore.findById(
