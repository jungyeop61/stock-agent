CREATE TABLE oto_conditional_order_previews (
    ...
    first_estimated_after_commission NUMERIC(65, 18) NOT NULL,
    ...
    second_estimated_after_commission NUMERIC(65, 18) NOT NULL,
    buying_power_checked BOOLEAN NOT NULL,
...
CREATE INDEX idx_oto_conditional_previews_status_expires_at
    ON oto_conditional_order_previews (status, expires_at);

CREATE TABLE oto_conditional_order_executions (
    ...
    version BIGINT NOT NULL DEFAULT 0,
    CONSTRAINT fk_oto_conditional_execution_preview
        FOREIGN KEY (preview_id) REFERENCES oto_conditional_order_previews (preview_id)
);

CREATE INDEX idx_oto_conditional_executions_status_updated_at
    ON oto_conditional_order_executions (status, updated_at);
