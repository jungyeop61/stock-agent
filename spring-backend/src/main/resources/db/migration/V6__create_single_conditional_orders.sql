CREATE TABLE single_conditional_order_previews (
    ...
    trigger_price NUMERIC(65, 18) NOT NULL,
    order_price NUMERIC(65, 18),
...
CREATE INDEX idx_single_conditional_previews_status_expires_at
    ON single_conditional_order_previews (status, expires_at);

CREATE TABLE single_conditional_order_executions (
    ...
    version BIGINT NOT NULL DEFAULT 0,
    CONSTRAINT fk_single_conditional_execution_preview
        FOREIGN KEY (preview_id) REFERENCES single_conditional_order_previews (preview_id)
);

CREATE INDEX idx_single_conditional_executions_status_updated_at
    ON single_conditional_order_executions (status, updated_at);
