...
/**
 * 사용자가 승인하기 전에 읽어 줄 OTO 조건 주문의 변경 불가 사본입니다.
 *
 ...
 * @param symbol 종목 코드
 * @param conditionalOrderType OTO 조건 주문 유형
 * @param quantity 두 조건이 공통으로 사용할 수량
 ...
 * @param commissionRate 계산에 사용한 매매 수수료율
 * @param first 먼저 발동할 매수 조건의 예상 결과
 * @param second 첫 매수 체결 뒤 감시할 매도 조건의 예상 결과
 * @param buyingPowerChecked 첫 매수의 수수료 포함 필요 금액을 확인했으면 true
 * @param sellTaxExcluded 후속 매도 예상 수령액에 매도 세금이 포함되지 않았으면 true
 ...
 */
public record OtoConditionalOrderPreviewResponse(
		String previewId,
	...
	public String toString() {
		return "OtoConditionalOrderPreviewResponse[previewId=***, accountSeq=***"
				...
				+ ", first=***, second=***, buyingPowerChecked=" + buyingPowerChecked
				+ ", sellTaxExcluded=" + sellTaxExcluded
	...
	/**
	 * 한 OTO 조건과 해당 조건 발동 시 예상되는 주문 결과를 표현합니다.
	 *
	 * @param side 매수 또는 매도 주문 방향
	 * @param triggerPrice 주문을 발동할 감시가격
	 * @param orderPrice 발동 후 제출할 지정가
	 * @param estimatedOrderAmount 예상 주문금액
	 * @param estimatedCommission 예상 매매 수수료
	 * @param estimatedAmountAfterCommission 매수는 수수료 포함 필요 금액, 매도는 수수료 차감 수령액
	 */
