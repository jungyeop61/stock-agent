...
/**
 * 단일 조건 주문 미리보기 한 건의 제출 상태를 데이터베이스에 보관합니다.
 */
@Entity
@Table(name = "single_conditional_order_executions")
class SingleConditionalOrderExecutionEntity {
	...
	protected SingleConditionalOrderExecutionEntity() {
	}

	/** 실행 준비 응답의 모든 값을 새 저장 객체에 기록합니다. */
	private SingleConditionalOrderExecutionEntity(
			SingleConditionalOrderExecutionResponse response) {
	...
	static SingleConditionalOrderExecutionEntity from(
			SingleConditionalOrderExecutionResponse response) {
		return new SingleConditionalOrderExecutionEntity(response);
	}

	/** 데이터베이스 값을 단일 조건 주문 실행 응답으로 변환합니다. */
	SingleConditionalOrderExecutionResponse toResponse() {
