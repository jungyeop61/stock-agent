...
import com.jusika.backend.conditionalordercreation.OcoConditionalOrderPreviewResponse.Condition;
...
/** OCO 조건 주문 미리보기의 두 조건·예상 금액·승인 상태를 데이터베이스에 보관합니다. */
@Entity
@Table(name = "oco_conditional_order_previews")
class OcoConditionalOrderPreviewEntity {
	...
	@Column(name = "first_estimated_proceeds", nullable = false, precision = 65, scale = 18)
	private BigDecimal firstEstimatedProceeds;
	@Enumerated(EnumType.STRING)
	...
	@Column(name = "second_estimated_proceeds", nullable = false, precision = 65, scale = 18)
	private BigDecimal secondEstimatedProceeds;
	...
	protected OcoConditionalOrderPreviewEntity() {
	}

	/** API OCO 미리보기 응답의 모든 변경 불가 값을 저장 객체에 복사합니다. */
	private OcoConditionalOrderPreviewEntity(OcoConditionalOrderPreviewResponse preview) {
		...
		this.firstEstimatedProceeds = preview.first().estimatedProceedsAfterCommission();
		...
		this.secondEstimatedProceeds = preview.second().estimatedProceedsAfterCommission();
	...
	static OcoConditionalOrderPreviewEntity from(OcoConditionalOrderPreviewResponse preview) {
		return new OcoConditionalOrderPreviewEntity(preview);
	}

	/** 데이터베이스 값을 OCO 조건 주문 미리보기 응답으로 변환합니다. */
	OcoConditionalOrderPreviewResponse toResponse() {
		return new OcoConditionalOrderPreviewResponse(
				...
				new Condition(firstSide, firstTriggerPrice, firstOrderPrice,
						firstEstimatedOrderAmount, firstEstimatedCommission, firstEstimatedProceeds),
				new Condition(secondSide, secondTriggerPrice, secondOrderPrice,
						secondEstimatedOrderAmount, secondEstimatedCommission, secondEstimatedProceeds),
