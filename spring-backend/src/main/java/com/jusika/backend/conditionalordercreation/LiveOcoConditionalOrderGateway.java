...
class LiveOcoConditionalOrderGateway implements OcoConditionalOrderGateway {
	...
	/** 계좌 식별값을 아는 실행 단계에서 OCO 생성 기능과 계좌 허용 목록을 함께 검사합니다. */
	@Override
	...
	/** OCO 종목이 시장별 LIVE 허용 목록에 등록됐는지 검사합니다. */
	@Override
	...
	/** 토스 호출 전에 OCO를 더할 활성 주문 여유를 다시 검사합니다. */
	@Override
	...
	/** 내부 실행 상태 생성 전에 OCO의 계좌·종목 1분 빈도를 검사합니다. */
	@Override
	...
	/** 토스 호출 직전에 OCO 생성 빈도를 멱등하게 예약합니다. */
	@Override
	...
	public ConditionalOrderCreationResponse submit(
		...
		reserveOrderRate(
				...
				"OCO_CONDITIONAL_ORDER:" + request.clientOrderId());
