...
/**
 * OCO 조건 주문의 향후 실제 생성 위치를 표시하되 현재는 전역 안전정책에서 항상 차단합니다.
...
class LiveOcoConditionalOrderGateway implements OcoConditionalOrderGateway {

	private final BrokerMutationSafetyPolicy safetyPolicy;

	/**
	 * 실제 OCO 생성 경계 진입을 최종 차단할 중앙 안전정책만 전달받습니다.
	 *
	 * @param safetyPolicy LIVE 기능 플래그, 긴급 차단 스위치와 어댑터 연결 상태 검사기
	 */
	LiveOcoConditionalOrderGateway(BrokerMutationSafetyPolicy safetyPolicy) {
	...
	/**
	 * 실제 클라이언트 호출 없이 중앙 안전정책을 다시 확인하고 OCO 조건 주문 생성을 차단합니다.
	 *
	 ...
	 *
	 * @return 민감정보가 없는 OCO 조건 주문 생성 차단 오류
	 */
	private BrokerMutationBlockedException clientNotConnectedException() {
		return new BrokerMutationBlockedException("OCO 조건 주문 LIVE 클라이언트가 연결되어 있지 않습니다.");
	}
}
