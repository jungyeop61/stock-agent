...
/**
 * OTO 조건 주문의 실제 토스 클라이언트 연결 위치이며 중앙 안전정책을 통과한 요청만 전달합니다.
 * 현재 운영 설정은 OTO 조건 주문 어댑터 준비 상태가 false이므로 클라이언트 호출 전에 차단됩니다.
 */
@Component
...
class LiveOtoConditionalOrderGateway implements OtoConditionalOrderGateway {
	...
	/** 내부 실행 상태 생성 전에 OTO의 계좌·종목 1분 빈도를 검사합니다. */
	@Override
	...
	/** 토스 호출 직전에 OTO 생성 빈도를 멱등하게 예약합니다. */
	@Override
	...
	public ConditionalOrderCreationResponse submit(
		...
		reserveOrderRate(
				...
				"OTO_CONDITIONAL_ORDER:" + request.clientOrderId());
		...
		try {
			ConditionalOrderCreationResponse response =
					conditionalOrderClient.createOtoConditionalOrder(accountSeq, request);
