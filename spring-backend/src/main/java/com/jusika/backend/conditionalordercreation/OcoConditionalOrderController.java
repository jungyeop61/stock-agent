...
/** OCO 조건 주문의 미리보기·승인·모의 실행·실행 결과 조회 주소를 제공합니다. */
@RestController
@RequestMapping("/api/conditional-orders/oco")
@RequiresInternalApiAuthority(InternalApiAuthority.ORDER)
public class OcoConditionalOrderController {

	private final OcoConditionalOrderService service;

	/**
	 * OCO 검증과 안전한 실행을 담당하는 서비스를 전달받습니다.
	 *
	 * @param service OCO 조건 주문 서비스
	 */
	public OcoConditionalOrderController(OcoConditionalOrderService service) {
		this.service = service;
	}

	/**
	 * 두 조건과 계좌의 매도 가능 수량을 확인하고 실제 주문 없이 미리보기를 만듭니다.
	 *
