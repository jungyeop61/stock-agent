...
/**
 * 단일 조건 주문 실행 행의 미리보기 잠금과 조건부 상태 변경을 수행합니다.
 */
interface SingleConditionalOrderExecutionJpaRepository
		extends JpaRepository<SingleConditionalOrderExecutionEntity, String> {

	/** 실행권 확인 중 대상 미리보기 행을 잠급니다. */
	@Query(value = "SELECT preview_id FROM single_conditional_order_previews "
			+ "WHERE preview_id = :previewId FOR UPDATE", nativeQuery = true)
	Optional<String> lockPreview(@Param("previewId") String previewId);

	/** 미리보기의 기존 실행 기록을 조회합니다. */
	Optional<SingleConditionalOrderExecutionEntity> findByPreviewId(String previewId);

	/** 준비 상태의 실행만 제출 중으로 변경합니다. */
	@Modifying(clearAutomatically = true, flushAutomatically = true)
	@Query("""
			update SingleConditionalOrderExecutionEntity execution
	...
	/** 예상 상태의 실행을 거절 또는 결과 불명 상태로 변경합니다. */
	@Modifying(clearAutomatically = true, flushAutomatically = true)
	@Query("""
			update SingleConditionalOrderExecutionEntity execution
