...
/** OCO 실행 행의 미리보기 잠금과 조건부 상태 변경을 수행합니다. */
interface OcoConditionalOrderExecutionJpaRepository
		extends JpaRepository<OcoConditionalOrderExecutionEntity, String> {

	/** 실행권 확인 중 대상 OCO 미리보기 행을 잠급니다. */
	@Query(value = "SELECT preview_id FROM oco_conditional_order_previews "
			+ "WHERE preview_id = :previewId FOR UPDATE", nativeQuery = true)
	Optional<String> lockPreview(@Param("previewId") String previewId);
	/** 미리보기의 기존 OCO 실행 기록을 조회합니다. */
	Optional<OcoConditionalOrderExecutionEntity> findByPreviewId(String previewId);

	/** 준비 상태의 OCO 실행만 제출 중으로 변경합니다. */
	@Modifying(clearAutomatically = true, flushAutomatically = true)
	@Query("""
			update OcoConditionalOrderExecutionEntity execution
	...
	/** 제출 중인 OCO 실행에 조건 주문 식별값을 기록하고 접수 상태로 변경합니다. */
	@Modifying(clearAutomatically = true, flushAutomatically = true)
	@Query("""
			update OcoConditionalOrderExecutionEntity execution
	...
	/** 예상 상태의 OCO 실행을 거절 또는 결과 불명 상태로 변경합니다. */
	@Modifying(clearAutomatically = true, flushAutomatically = true)
	@Query("""
			update OcoConditionalOrderExecutionEntity execution
