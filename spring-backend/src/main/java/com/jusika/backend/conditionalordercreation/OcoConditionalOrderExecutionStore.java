...
/** OCO 조건 주문 실행의 중복 방지와 상태 저장 기능을 서비스에서 분리합니다. */
interface OcoConditionalOrderExecutionStore {
	/** 미리보기 한 건당 한 번만 OCO 실행권을 확보합니다. */
	boolean claim(OcoConditionalOrderExecutionResponse execution);
	...
	/** OCO 조건 주문 접수 여부를 알 수 없는 상태로 변경합니다. */
	boolean markUnknown(String executionId, OffsetDateTime failedAt);
	/** 실행 식별값으로 저장된 OCO 실행 기록을 조회합니다. */
	Optional<OcoConditionalOrderExecutionResponse> findById(String executionId);
	/** 미리보기 식별값으로 저장된 OCO 실행 기록을 조회합니다. */
	Optional<OcoConditionalOrderExecutionResponse> findByPreviewId(String previewId);
}
