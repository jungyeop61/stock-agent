...
/**
 * 단일 조건 주문 미리보기 행을 저장하고 승인·만료·사용 상태를 조건부로 변경합니다.
 */
interface SingleConditionalOrderPreviewJpaRepository
		extends JpaRepository<SingleConditionalOrderPreviewEntity, String> {

	/** 유효시간 안의 승인 대기 미리보기만 승인합니다. */
	@Modifying(clearAutomatically = true, flushAutomatically = true)
	@Query("""
			update SingleConditionalOrderPreviewEntity preview
	...
	/** 승인된 미리보기만 조건 주문 실행에 사용된 상태로 변경합니다. */
	@Modifying(clearAutomatically = true, flushAutomatically = true)
	@Query("""
			update SingleConditionalOrderPreviewEntity preview
