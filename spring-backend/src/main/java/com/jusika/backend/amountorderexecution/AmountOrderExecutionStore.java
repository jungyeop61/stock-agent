...
/**
 * 금액 주문 실행권을 한 번만 확보하고 실행 상태를 조건부 변경하는 저장소 경계입니다.
 */
public interface AmountOrderExecutionStore {

	/**
	 * 금액 미리보기 행을 잠근 뒤 기존 실행이 없을 때만 실행 준비 기록을 만듭니다.
	 *
	 * @param execution 저장할 실행 준비 기록
	 * @param requestFingerprint 최초 금액 주문 본문의 변경 감지용 지문
	 * @return 이번 호출이 실행권을 확보했으면 true
	 */
	boolean claim(AmountOrderExecutionResponse execution, String requestFingerprint);

	/**
	 * 실행 준비 상태를 금액 주문 제출 중 상태로 변경합니다.
	 *
	...
	/**
	 * 제출 전 내부 상태가 어긋난 실행 준비 기록을 안전하게 종료합니다.
	 *
	 * @param executionId 변경할 실행 식별값
	 * @param failedAt 내부 오류를 확인한 시각
	 * @return 이번 호출이 상태를 변경했으면 true
	 */
	boolean markPreparationFailed(String executionId, OffsetDateTime failedAt);

	/**
	 * 제출 중 실행을 접수 상태로 변경합니다.
	 *
	 * @param executionId 변경할 실행 식별값
	 ...
	 * @return 저장된 실행 기록이며 없으면 빈 값
	 */
	Optional<AmountOrderExecutionResponse> findById(String executionId);

	/**
	 * 미리보기 식별값으로 기존 금액 주문 실행 기록을 조회합니다.
	 *
	 * @param previewId 조회할 미리보기 식별값
	 * @return 저장된 실행 기록이며 없으면 빈 값
	 */
	Optional<AmountOrderExecutionResponse> findByPreviewId(String previewId);
}
