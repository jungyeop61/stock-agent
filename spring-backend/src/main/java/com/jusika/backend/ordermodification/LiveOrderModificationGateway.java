...
/**
 * 일반 미체결 주문 정정의 향후 실제 실행 위치를 표시하되 현재는 전역 안전정책에서 항상 차단합니다.
 * 토스 주문 클라이언트를 의존성으로 받지 않으므로 이 구현만으로 실제 주문을 정정할 수 없습니다.
 */
@Component
...
class LiveOrderModificationGateway implements OrderModificationGateway {
	...
	/**
	 * 실제 정정 경계 진입을 최종 차단할 중앙 안전정책만 전달받습니다.
	 *
	 * @param safetyPolicy LIVE 기능 플래그, 긴급 차단 스위치와 어댑터 연결 상태 검사기
	 */
	LiveOrderModificationGateway(BrokerMutationSafetyPolicy safetyPolicy) {
	 ...
	 *
	 * @return 민감정보가 없는 실제 주문 정정 차단 오류
	 */
	private BrokerMutationBlockedException clientNotConnectedException() {
		return new BrokerMutationBlockedException("일반 주문 정정 LIVE 클라이언트가 연결되어 있지 않습니다.");
	}
}
