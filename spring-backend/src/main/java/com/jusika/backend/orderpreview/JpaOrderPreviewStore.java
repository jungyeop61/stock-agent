...
/**
 * 스프링 데이터 JPA를 사용해 주문 미리보기 저장소 경계를 구현합니다.
 */
@Repository
class JpaOrderPreviewStore implements OrderPreviewStore {

	private final OrderPreviewJpaRepository repository;

	/**
	 * 실제 데이터베이스 접근을 담당하는 JPA 저장소를 전달받습니다.
	 *
	 * @param repository 주문 미리보기 JPA 저장소
	 */
	JpaOrderPreviewStore(OrderPreviewJpaRepository repository) {
		this.repository = repository;
	}

	/**
	 * 주문 미리보기 전체 내용을 새 데이터베이스 행으로 저장합니다.
	 *
	...
	public OrderPreviewResponse save(OrderPreviewResponse preview) {
		return repository.save(OrderPreviewEntity.from(preview)).toResponse();
	}

	/**
	 * 승인 가능한 행만 조건부 갱신해 동시에 두 번 승인되는 일을 막습니다.
	 *
	 ...
	 * @return 저장된 주문 미리보기이며 없으면 빈 값
	 */
	@Override
	@Transactional(readOnly = true)
	public Optional<OrderPreviewResponse> findById(String previewId) {
		return repository.findById(previewId).map(OrderPreviewEntity::toResponse);
	}
}
