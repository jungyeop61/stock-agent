...
/** JPA를 사용해 조건 주문 취소 미리보기 저장소 경계를 구현합니다. */
@Repository
class JpaConditionalOrderCancellationPreviewStore
		implements ConditionalOrderCancellationPreviewStore {

	private final ConditionalOrderCancellationPreviewJpaRepository repository;

	/** 조건 주문 취소 미리보기 JPA 저장소를 전달받습니다. */
	JpaConditionalOrderCancellationPreviewStore(
			ConditionalOrderCancellationPreviewJpaRepository repository) {
		this.repository = repository;
	}

	/** 새 조건 주문 취소 미리보기의 변경 불가 내용을 저장합니다. */
	@Override
	@Transactional
	public ConditionalOrderCancellationPreviewResponse save(
			ConditionalOrderCancellationPreviewResponse preview) {
		return repository.save(
				ConditionalOrderCancellationPreviewEntity.from(preview)).toResponse();
	}
	...
	/** 식별값으로 저장된 조건 주문 취소 미리보기를 조회합니다. */
	@Override
	@Transactional(readOnly = true)
	public Optional<ConditionalOrderCancellationPreviewResponse> findById(String previewId) {
		return repository.findById(previewId)
				.map(ConditionalOrderCancellationPreviewEntity::toResponse);
	}
}
