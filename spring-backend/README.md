...
승인된 미리보기와 주문 클라이언트는 아직 연결하지 않아 외부에서 실제 주문을 실행할 수는 없습니다.
...
이 클라이언트는 아직 HTTP 컨트롤러나 승인된 미리보기와 연결되어 있지 않으므로 애플리케이션 외부에서 실제 주문을 실행할 수 없습니다.
...
다음 단계에서 동일한 `clientOrderId`로 주문 접수 여부를 조회하고, 승인된 미리보기를 정확히 한 번만 실행하는 흐름에 연결합니다.
