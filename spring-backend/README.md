...
승인된 주문 실행·복구·취소·정정과 조건 주문 생성·취소·정정에는 모두 차단된 LIVE 안전 경계가 있습니다. 일반 수량·금액 주문, 일반 주문 취소·정정과 SINGLE·OCO 조건 주문 생성은 토스 클라이언트 연결 코드까지 준비했지만 운영 준비 상태가 미연결로 고정되어 주문을 생성·취소·정정할 수 없습니다.
...
- 사전 검사가 실패하면 승인된 미리보기는 소비되지 않고 실행 기록과 멱등성 식별값도 생성되지 않습니다.
- 제출 메서드 내부에서도 중앙 정책을 다시 확인합니다.
- 실제 어댑터 연결값은 계속 `false`이고 토스 클라이언트도 없으므로 실제 OTO 생성 요청은 발생하지 않습니다.
- 생성 결과가 `UNKNOWN`인 경우 자동 재시도나 자동 복구 경로는 계속 제공하지 않습니다.

이번 변경은 OTO 조건 주문의 조건부 의존성 선택과 상태 변경 전 차단 순서만 추가합니다.
모든 주문 변경 흐름에 LIVE 안전 경계가 있지만 실제 어댑터는 연결되지 않아 전체 애플리케이션의 LIVE 모드는 사용할 수 없습니다.
데이터베이스 구조 변경은 없습니다.
