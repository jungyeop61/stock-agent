...
승인된 주문 실행·복구·취소·정정과 조건 주문 생성·취소·정정에는 모두 차단된 LIVE 안전 경계가 있습니다. 일반 수량·금액 주문, 일반 주문 취소·정정, SINGLE·OCO·OTO 조건 주문 생성과 조건 주문 취소는 토스 클라이언트 연결 코드까지 준비했지만 운영 준비 상태가 미연결로 고정되어 주문을 생성·취소·정정할 수 없습니다.
...
- 토스 클라이언트의 확정 거절과 결과 불명 상태는 실행 서비스가 각각 `REJECTED`와 `UNKNOWN`으로 저장할 수 있도록 변환합니다.
- 복구 메서드는 최초와 같은 `clientOrderId`와 주문 본문을 전달하도록 준비했지만 현재 복구 서비스는 `MOCK` 실행만 허용합니다.
...
- LIVE 게이트웨이는 `TossConditionalOrderClient`나 토스 인증 객체를 생성자와 필드로 받지 않습니다.
- 승인된 정정 실행은 최신 원조건 주문·현재가·매수 가능 금액·매도 가능 수량·수수료를 재조회하기 전에 중앙 LIVE 안전정책을 확인합니다.
- 사전 검사가 실패하면 승인된 정정 미리보기는 소비되지 않고 실행 기록도 생성되지 않습니다.
- 정정 메서드 내부에서도 중앙 정책을 다시 확인합니다.
- 토스 조건 주문 정정은 기존 주문을 취소하고 새 조건 주문 식별값을 발급하지만, 현재 LIVE 경계는 두 동작 모두 시작할 수 없습니다.
- 실제 어댑터 연결값은 계속 `false`이고 토스 클라이언트도 없으므로 실제 조건 주문 정정 요청은 발생하지 않습니다.
- 정정 결과가 `UNKNOWN`인 경우 자동 재시도 경로는 계속 제공하지 않습니다.

이번 변경으로 모든 주문 변경 흐름의 LIVE 안전 경계 골격이 갖춰졌습니다.
다만 실제 토스 클라이언트 연결 전에는 전체 애플리케이션의 LIVE 모드를 사용할 수 없습니다.
데이터베이스 구조 변경은 없습니다.
...
LIVE 복구 메서드에도 최초 요청을 토스 클라이언트에 전달하는 코드를 준비했지만, 현재 복구 서비스는 최초 실행과 현재 경계가 모두 `MOCK`일 때만 복구를 허용합니다.
